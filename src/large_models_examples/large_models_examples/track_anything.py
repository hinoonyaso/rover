...
class ObjectTracker:
    def __init__(self, use_mouse=False, automatic=False, log=None): 
        ...
        self.linear_pid = pid.PID(0.0, 0.0, 0.0)# pid initialization(pid初始化)
    ...
    def update_pid(self, p1, p2):
        self.linear_pid = pid.PID(p1[0], p1[1], p1[2])# pid initialization(pid初始化)
    ...
    def onmouse(self, event, x, y, flags, param):
        ...
        if self.drag_start:  # Whether mouse dragging has started; record mouse position (是否开始拖动鼠标，记录鼠标位置)
